...
class RequestAddNavItem extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      showing: false,
  ...
  async handleSubmit(e) {
    e.preventDefault();
    this.hideModal();
    const form = document.forms.requestAdd;
    const issue = {
    ...
    // TODO: Update issue to request in #Iter2
    const data = await graphQLFetch(query, { issue }, showError);
    if (data) {
      const { history } = this.props;
      // TODO: Update issueAdd to requestAdd in #Iter2
      ...
      <React.Fragment>
        <NavItem disabled={!signedIn} onClick={this.showModal}>
          <OverlayTrigger
            placement="left"
            delayShow={1000}
            overlay={<Tooltip id="create-request">Create Request</Tooltip>}
          >
...
export default withToast(withRouter(RequestAddNavItem));
